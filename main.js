...
const { viewOnceCommand, handleOwnerViewOnceReply, cacheIncomingViewOnce } = require('./commands/viewonce');
...
const { addCommandReaction, handleAreactCommand, handleAutoReaction } = require('./lib/reactions');
...
const { botToggleCommand } = require('./commands/botToggle');
...
async function handleMessages(sock, messageUpdate, printLog) {
    ...
    try {
        ...
        // Check global bot active status
        const botActive = isBotActive();
        if (!botActive) {
            if (userMessage.startsWith('.bot')) {
                await botToggleCommand(sock, chatId, message, userMessage);
            }
            return; // Completely paused and silent when bot is globally OFF
        }
        ...
        // Cache view-once media if present
        await cacheIncomingViewOnce(sock, message);

        // Global auto-reaction with random emojis on all incoming messages
        ...
        // View-once auto-reveal: If owner replies to any view-once media, extract and send directly to owner's DM
        if (isOwnerOrSudoCheck) {
            const rawReplyText = rawText || userMessage;
            const handledViewOnce = await handleOwnerViewOnceReply(sock, message, chatId, rawReplyText);
            if (handledViewOnce) {
                return;
            }
        }
        ...
        const ownerCommands = ['.bot', '.disconnect', '/disconnect', '.sessions', '/sessions', '.makeowner', '/makeowner', '.mode', '.autostatus', '.antidelete', '.cleartmp', '.setpp', '.clearsession', '.areact', '.autoreact', '.autoseen', '.autotyping', '.autoread', '.pmblocker', '.wow', '.vv', '.backup', '.gdrive'];
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.unban'):
                ...
                break;
            case userMessage === '.help' || userMessage === '.menu' || userMessage === '.bot' || userMessage === '.list' || userMessage.startsWith('.menu ') || userMessage.startsWith('.help '):
                const categoryArg = userMessage.split(/\s+/)[1] || null;
                await helpCommand(sock, chatId, message, categoryArg);
                commandExecuted = true;
                break;
            ...
            case userMessage === '.wow' || userMessage === '.vv':
                await viewOnceCommand(sock, chatId, message);
                break;
            case userMessage === '.backup' || userMessage === '.gdrive':
            ...
            case userMessage === '.waifu' || userMessage.startsWith('.waifu '):
                {
                    ...
                    await waifuCommand(sock, chatId, message, args);
                    commandExecuted = true;
                }
                break;
            case userMessage === '.maid' || userMessage.startsWith('.maid '):
                await waifuCommand(sock, chatId, message, [], 'maid');
                commandExecuted = true;
                break;
            case userMessage === '.uniform' || userMessage.startsWith('.uniform '):
            case userMessage === '.cosplay' || userMessage.startsWith('.cosplay '):
                await waifuCommand(sock, chatId, message, [], 'uniform');
                commandExecuted = true;
                break;
            case userMessage === '.selfie' || userMessage.startsWith('.selfie ') || userMessage === '.selfies' || userMessage.startsWith('.selfies '):
                await waifuCommand(sock, chatId, message, [], 'selfies');
                commandExecuted = true;
                break;
            case userMessage === '.marin' || userMessage.startsWith('.marin ') || userMessage.startsWith('.marinkitagawa'):
                await waifuCommand(sock, chatId, message, [], 'marin');
                commandExecuted = true;
                break;
            case userMessage === '.mori' || userMessage.startsWith('.mori ') || userMessage.startsWith('.moricalliope'):
                await waifuCommand(sock, chatId, message, [], 'mori');
                commandExecuted = true;
                break;
            case userMessage === '.raiden' || userMessage.startsWith('.raiden ') || userMessage.startsWith('.raidenshogun'):
                await waifuCommand(sock, chatId, message, [], 'raiden');
                commandExecuted = true;
                break;
            case userMessage === '.ayaka' || userMessage.startsWith('.ayaka ') || userMessage.startsWith('.kamisato'):
                await waifuCommand(sock, chatId, message, [], 'ayaka');
                commandExecuted = true;
                break;
            case userMessage === '.rem' || userMessage.startsWith('.rem '):
                await waifuCommand(sock, chatId, message, [], 'rem');
                commandExecuted = true;
                break;
            case userMessage === '.genshin' || userMessage.startsWith('.genshin ') || userMessage.startsWith('.genshinimpact'):
                await waifuCommand(sock, chatId, message, [], 'genshin');
                commandExecuted = true;
                break;
            case userMessage === '.oppai' || userMessage.startsWith('.oppai '):
                await waifuCommand(sock, chatId, message, [], 'oppai');
                commandExecuted = true;
                break;
            case userMessage === '.onepiece' || userMessage.startsWith('.onepiece '):
                await waifuCommand(sock, chatId, message, [], 'onepiece');
                commandExecuted = true;
                break;
            case userMessage === '.nsfw' || userMessage.startsWith('.nsfw ') || userMessage === '.nswf' || userMessage.startsWith('.nswf '):
                {
                    const parts = userMessage.trim().split(/\s+/);
                    const subTag = parts[1] || null;
                    await waifuCommand(sock, chatId, message, subTag ? [subTag] : [], subTag ? null : 'nsfw');
                    commandExecuted = true;
                }
                break;
            case userMessage === '.ero' || userMessage.startsWith('.ero '):
                await waifuCommand(sock, chatId, message, [], 'ero');
                commandExecuted = true;
                break;
            case userMessage === '.ecchi' || userMessage.startsWith('.ecchi '):
                await waifuCommand(sock, chatId, message, [], 'ecchi');
                commandExecuted = true;
                break;
            case userMessage === '.hentai' || userMessage.startsWith('.hentai '):
                await waifuCommand(sock, chatId, message, [], 'hentai');
                commandExecuted = true;
                break;
            case userMessage === '.milf' || userMessage.startsWith('.milf '):
                await waifuCommand(sock, chatId, message, [], 'milf');
                commandExecuted = true;
                break;
            case userMessage === '.ass' || userMessage.startsWith('.ass '):
                await waifuCommand(sock, chatId, message, [], 'ass');
                commandExecuted = true;
                break;
            case userMessage === '.paizuri' || userMessage.startsWith('.paizuri '):
                await waifuCommand(sock, chatId, message, [], 'paizuri');
                commandExecuted = true;
                break;
            case userMessage === '.oral' || userMessage.startsWith('.oral '):
                await waifuCommand(sock, chatId, message, [], 'oral');
                commandExecuted = true;
                break;
            case userMessage === '.nami' || userMessage.startsWith('.nami '):
                await waifuCommand(sock, chatId, message, [], 'nami');
                commandExecuted = true;
                break;
            case userMessage === '.anime' || userMessage.startsWith('.anime '):
                {
                    const parts = userMessage.trim().split(/\s+/);
                    const sub = parts[1] ? parts[1].toLowerCase() : '';
                    if (!sub) {
                        await waifuCommand(sock, chatId, message, ['list']);
                    } else {
                        const { getCategoryConfig } = require('./commands/waifu');
                        const waifuCfg = getCategoryConfig(sub);
                        if (waifuCfg) {
                            await waifuCommand(sock, chatId, message, [sub]);
                        } else {
